...
export const getSharedTrip = async (req: Request, res: Response) => {
  try {
    const trip = await shareService.getSharedTrip(req.params.shareToken as string);
...
export const shareTrip = async (req: Request, res: Response) => {
  try {
    const share = await shareService.shareTrip(req.params.id as string, req.user!.userId, req.body);
