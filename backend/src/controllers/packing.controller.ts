...
export const getPackingItems = async (req: Request, res: Response) => {
  try {
    const data = await packingService.getPackingItems(req.params.id as string, req.user!.userId);
...
export const createPackingItem = async (req: Request, res: Response) => {
  try {
    const item = await packingService.createItem(req.params.id as string, req.user!.userId, req.body);
...
export const togglePackingItem = async (req: Request, res: Response) => {
  try {
    const item = await packingService.toggleItem(req.params.id as string, req.params.itemId as string, req.user!.userId);
...
export const deletePackingItem = async (req: Request, res: Response) => {
  try {
    await packingService.deleteItem(req.params.id as string, req.params.itemId as string, req.user!.userId);
...
export const resetPacking = async (req: Request, res: Response) => {
  try {
    await packingService.resetAll(req.params.id as string, req.user!.userId);
